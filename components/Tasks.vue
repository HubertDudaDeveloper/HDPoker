<template>
  <div class="tasks">
    <div class="tasks-header">
      <h2>Tasks</h2>
      <button class="button" @click="handleAddTask">Add new</button>
    </div>
    <div class="tabs-wrapper">
      <div
        class="tab"
        :class="{ glass: tasksTab === 'active' }"
        @click="handleTab"
      >
        Active
      </div>
...
const tasks = computed(() =>
  tasksTab.value === "active" ? activeTasks.value : finishedTasks.value
);
...
const handleSendNewTask = async () => {
  ...
  await pokerState.value.ws!.send(
    `${JSON.stringify({
      ...
      room: { ...roomState.value.room, tasks: [...oldTasks, newTask.value] },
    })}`
  );
