  ...
  <div class="wrapper">
    <form class="data-container">
      ...
      <input
        id="room"
        v-model="room"
        :class="{ 'is-error': hasTriedToSend && !room }"
      ...
      <input
        ...
        v-model="userName"
        :class="{ 'is-error': hasTriedToSend && !userName }"
...
const hasTriedToSend = ref(false);
...
const getUser = (): User => {
  ...
  return isSavedUser
    ? savedUser
    : { id: "init", name: userName.value, points: 0, image: "" };
};
...
const checkRoom = async (type: string) => {
  const roomInfo: string = await $fetch(
    "https://hd-dev.pl/poker/api/room-info",
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        type: type,
        room: { name: room.value, password: password.value },
      }),
    }
  );
  return roomInfo;
...
const handleJoinRoom = async () => {
  ...
  hasTriedToSend.value = true;

  if (!room.value || !(getUser().name && userName.value)) {
    return;
  }
  ...
  if (JSON.parse(roomInfo).response) {
    ...
    roomStore.handleJoinRoom(
      roomData,
      { ...getUser(), image: userImage.value },
      pokerState.value.ws!
    );
